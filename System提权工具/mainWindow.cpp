...
VOID setWindowIcon(HWND hwnddlg, DWORD bigiconCode, DWORD smalliconCode) {
	HINSTANCE hinstance = ::GetModuleHandle(NULL);
	HICON HiconBig = LoadIcon(hinstance, MAKEINTRESOURCE(bigiconCode));
	HICON HiconSmall = LoadIcon(hinstance, MAKEINTRESOURCE(smalliconCode));
	SendMessage(hwnddlg, WM_SETICON, ICON_BIG, (LONG64)HiconBig);
	SendMessage(hwnddlg, WM_SETICON, ICON_SMALL, (LONG64)HiconSmall);
}
...
INT_PTR CALLBACK mainwindow(HWND hwnddlg, UINT umsg, WPARAM wparam, LPARAM lparam) {
	...
	case WM_DROPFILES:{
		...
		for (UINT i = 0; i < fileCount; i++) {
			...
			DWORD filetype = 0;
			BOOL isPE =  GetBinaryTypeW(g_FileName, &filetype);
			if (isPE == FALSE) {
	...
	case WM_COMMAND: {
		...
		case IDC_BUTTON1: {
			...
			DWORD filetype = NULL;
			if (!GetBinaryTypeW(g_FileName, &filetype)) {
				MessageBox(hwnddlg, L"The file suffix must be. exe", L"WARNING", MB_OK | MB_ICONEXCLAMATION);
				RtlZeroMemory(g_FileName, MAX_PATH * sizeof(WCHAR));
				return FALSE;
			}
